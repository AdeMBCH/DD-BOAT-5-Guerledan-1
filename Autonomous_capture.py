...
def fit_ellispoide_sphere(M):
    ...
    def Vect(p):
        ...
        a = np.array([p[6], p[7], p[8]], dtype=float)
        c = float(p[9])
        return Q, a, c

    # Beaucoup d'Exception et de tests ici pour débugger
    def try_params(p):
        Q, a, c = Vect(p)
        try:
            b = -0.5 * np.linalg.solve(Q, a)
        except Exception:
            return None
        denom = float(np.dot(b, np.dot(Q, b)) - c)
        if abs(denom) < 1e-18:
            return None
        k = 1.0 / denom
        Q_norm = Q * k
        try:
            L = np.linalg.cholesky(Q_norm)
        except Exception:
            return None
        A = L.T
        return b, A
...
# Récupère les données de datacalibration.txt
...
class FFCtrl(object):
    ...
    def step(self, u_d, omega_d, psi_d):
        # FFC avec correction en cap, saturation tanh
        psi = self.read_heading_rad()
        e_psi = sawtooth(psi - psi_d) # sawtooth
...
# Lecture GPS, code majoritairement de M.ZERR
        ...
        if ns is not None:
            if last is None:
                ...
                if abs(dx) + abs(dy) > 0.2: # > 20 cm
                    ...
                    return ns
        time.sleep(0.2)
    raise RuntimeError("GPS: pas de données GLL dans le délai")

#Fonction pour aller sur un point GPS
def goto_point(ctrl, gps, goal_xy, u_d, radius_m, timeout_s, seg=None):
    ...
    while True:
        ...
        if dist <= radius_m:
            ...
            return ns
        #print(bearing_geo_from_vec(dx, dy)) #Pour vérifier visuellement le heading
...
# Pour rétrocompatibilité avec fonctions précédentes
...
# Passage en repère du lac
def local_xy(long_deg, lat_deg, long0_deg, lat0_deg, R=6378137.0): #Hypothèse sur R pris comme rayon de la Terre
...
class PreyOrbit(object):
    ...
    @staticmethod #Passage en statique pour pouvoir l'utiliser en dehors
    def R2(t):
    ...
    def desired_pos(self, t):
        ...
        R2t = self.R2(t)
        w2 = self.w2(t)
        #return (px,py) # Si on veut être la proie
        return (px + R2t * math.cos(w2*t+ 2.0 *math.pi*5/self.N),
                 py + R2t * math.sin(w2*t+ 2.0 *math.pi*5/self.N)) #Si on veut être le prédateur
...
def track_at(ctrl, gps, traj, t_start, u_d_nom, k_pos=0.8, dt_cmd=0.1, duration_s=750.0, seg_robot=None, seg_ref=None):
    ...
    while True:
        ...
        if psi_d_prev is None:
            ...
            dpsi = sawtooth(psi_d - psi_d_prev)
            dt = max(0.001, (time.time() - t_prev))
            omega_d = dpsi / dt
...
# ---------- Main ----------
def main():
    duree=750.0 #Si modifié, ne pas oublier le default dans track_at

    p = argparse.ArgumentParser(description="Suivi proie avec FFC")
    ...
    # FFC
    p.add_argument('--u_d', type=float, default=2, help='Commande moteur normalisée [-1,1]')
        #----- coefficient pour comportement de la puissance moteur-----
    ...
    p.add_argument('--mmax', type=int, default=200)
        #----------------------------------------------------------------
    p.add_argument('--k_pos', type=float, default=0.8, help='Gain position du cap')
    ...
    p.add_argument('--arrive_radius', type=float, default=3.0) #Rayon acceptable pour l'arrivée sur un point
    ...
    # Aller au point de départ de la proie p(0) = C + [R1, 0]
    # p0x, p0y = traj.proie(0.0)
    ...
    # ----- Code généré par ChatGPT pour obtenir la trace GPS même après arrêt abrupt (CTRL C, terminal coupé...)---

...
# ---------- FFC ----------
class FFCtrl(object):
    def __init__(self, imu, ard, A_mag=None, b_mag=None, alpha_v=3.0, alpha_w=10, kp_psi=10, mmax=230, lam=3.0):
        self.imu = imu
        self.ard = ard
...
def track_at(ctrl, gps, traj, t_start, u_d_nom, k_pos=0.8, dt_cmd=0.1, duration_s=750.0, seg_robot=None, seg_ref=None):
    ...
    while True:
        ...
        dist = np.hypot(ex,ey)
        print(dist)
        if dist <5 : ### ralentir quand on est proche de la position cible. Peut être amélioré largement en terme de forme et de coefficients
            u_d = 0.5*u_d_nom-u_d_nom*0.5*(dist/5) 
            ctrl.step(u_d=u_d, omega_d=omega_d, psi_d=psi_d)
        else :      ### avance normale
...
def main():
    ...
    p.add_argument('--N', type=int, default= 12, help='N pour le déphasage 2π*5/N')
